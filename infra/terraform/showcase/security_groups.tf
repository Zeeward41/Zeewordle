...
resource "aws_vpc_security_group_egress_rule" "allow_outbound_traffic_to_app_frontend" {
  # Allow outbound traffic to APP (port 3000) on target SG
  security_group_id = aws_security_group.monitoring_proxy.id
  description       = "Allow outbound traffic to APP on Frontend"
...
resource "aws_vpc_security_group_egress_rule" "allow_outbound_traffic_to_app_backend" {
  # Allow outbound traffic to APP (port 5000) on target SG
  security_group_id = aws_security_group.monitoring_proxy.id
  description       = "Allow outbound traffic to APP on backend"
